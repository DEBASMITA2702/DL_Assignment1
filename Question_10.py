...
if __name__ == '__main__':
    ...
    # flatten the input and one hot encode the output
    ...
    # create the object of the model and call the fitting function
    ...
    # get the predictions of the test images by running forward propagation with the weights and biases learned
    predictions = list()
    ...
    # create the object of the model and call the fitting function
    Second_model = NeuralNetwork(x_train, y_train, x_val, y_val, epochs = 20, num_hidden_layers = 3, neurons_in_hidden_layer = 128, 
                                initialization_method = "he_uni", activation_function = "relu", loss_function = "cross_entropy", 
                                learning_rate = 0.005, weight_decay = 0.0005)

    learned_weights, learned_biases = Second_model.fitModel(beta = 0.999, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-5, optimizer = "nadam", 
    ...
    for point in range(data_size):
        _, _, predictedValues = forwardPropagationForPredictions(learned_weights, learned_biases, x_test_flattened[point], 
                                                                Second_model.num_total_layers, 
                                                                Second_model.activation_function)
        predictions.append(predictedValues)
    ...
    # create the object of the model and call the fitting function
    Third_model = NeuralNetwork(x_train, y_train, x_val, y_val, epochs = 20, num_hidden_layers = 5, neurons_in_hidden_layer = 32, 
                                initialization_method = "he_nor", activation_function = "relu", loss_function = "cross_entropy", 
                                learning_rate = 0.005, weight_decay = 0)

    learned_weights, learned_biases = Third_model.fitModel(beta = 0.999, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-5, optimizer = "nadam", 
                        batch_size = 256, mnistTestRun = True)
    ...
    for point in range(data_size):
        _, _, predictedValues = forwardPropagationForPredictions(learned_weights, learned_biases, x_test_flattened[point], 
                                                                Third_model.num_total_layers, 
                                                                Third_model.activation_function)
        predictions.append(predictedValues)
